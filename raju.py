...
def analyze_sentiment(text):
    ...
    if sum(1 for w in bearish if w in text) > sum(1 for w in bullish if w in text):
        return " Bearish"
    return " Neutral"@st.cache_data(ttl=45)
def fetch_all_market_data():
    tickers_list = list(ALL_TICKERS.values())
    ticker_to_label = {v: k for k, v in ALL_TICKERS.items()}daily = yf.download(tickers=tickers_list, period="60d", interval="1d", progress=False)
...
return pd.DataFrame(rows)@st.cache_data(ttl=300)
def get_ticker_news(symbol):
    try:
        return yf.Ticker(symbol).news[:3]
    except:
        return []@st.cache_data(ttl=600)
...
def fetch_earnings_and_ratings(days_window=7):
    earnings_list = []
    ratings_list = []
    today = datetime.date.today()for label, symbol in ALL_TICKERS.items():
    try:
        ...
        if cal is not None and 'Earnings Date' in cal:
            ...
            for e_dt in e_dates:
                ...
                if abs((e_date - today).days) <= days_window:
                    news = ticker.news[:3]
                    sent = analyze_sentiment(news[0].get('title', '')) if news else " Neutral"
...
return (pd.concat(ratings_list) if ratings_list else pd.DataFrame()), pd.DataFrame(earnings_list)@st.cache_data(ttl=300, show_spinner="Fetching options chains...")
def get_options_pcr():
    res = {}
    for label, sym in OPTIONS_TICKERS.items():
        try:
            ...
            sent = (" Strongly Bullish" if pcr < 0.75 else " Bullish" if pcr < 0.90 else
                    " Neutral" if pcr < 1.10 else " Bearish" if pcr < 1.30 else " Strongly Bearish")
...
def color_pct(val):
    ...
    return 'color: #00ff00' if val > 0 else 'color: #ff4b4b' if val < 0 else ''def color_rel(val):
    if pd.isna(val): return ''
...
if not news_data.empty:
    ...
    st.sidebar.info("News unavailable")st.sidebar.title(" Market Settings")
refresh = st.sidebar.number_input('Refresh rate (sec)', 15, 600, 30)
st_autorefresh(interval=refresh * 1000, key="refresh")# ========================== DATA ==========================
est = pytz.timezone('US/Eastern')
time_now = datetime.datetime.now(est).strftime('%H:%M:%S')full_market = fetch_all_market_data().dropna(subset=['Change %'])global_df = full_market[full_market['Asset'].isin(GLOBAL_TICKERS.keys())].copy()
...
mag7_df   = full_market[full_market['Asset'].isin(MAG7_TICKERS.keys())].copy()for df in [global_df, sector_df, etf_df, tf_df, mag7_df]:
    df.sort_values('Change %', ascending=False, inplace=True)benchmark_change = full_market.loc[full_market['Asset'] == "SPY (S&P 500 ETF)", 'Change %'].iloc[0] if "SPY (S&P 500 ETF)" in full_market['Asset'].values else 0.0
for df in [sector_df, etf_df, tf_df, mag7_df]:
    df['RS'] = df['Change %'] - benchmark_changetop_gainers = full_market.nlargest(6, 'Change %')
top_losers  = full_market.nsmallest(6, 'Change %')# Mover news
mover_sent = {}
mover_news = {}
for _, row in pd.concat([top_gainers, top_losers]).drop_duplicates('Asset').iterrows():
    ...
    if not items:
        ...
        mover_sent[row['Asset']] = " Bullish" if sents.count(" Bullish") > sents.count(" Bearish") else " Bearish" if sents.count(" Bearish") > sents.count(" Bullish") else " Neutral"# ========================== MAIN UI ==========================
st.title(" Pro Market Terminal")
st.caption(f"Live • EST {time_now} • Refresh {refresh}s")# Scanner
...
with c3:
    ...
    st.metric("Breadth", f"{up} ↑ / {len(full_market)-up} ↓", delta=up-(len(full_market)-up))st.divider()# TABS
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    ...
    for name, df in [("Sectors", sector_df), ("24h & Commodities", tf_df), ("Mag7", mag7_df)]:
        ...
        st.divider()st.subheader("Intraday Charts")
...
for lab in sel:
    ...
    if not data.empty:
        st.write(f"**{lab}**")
        st.line_chart(data['Close'].tz_convert('US/Eastern'))with tab4:
    ...
    for i, (label, info) in enumerate(data.items()):
        ...
        if "error" in info:
            ...
            c.metric(label, f"{info['pcr']:.2f}", info['sentiment'],
                     help=f"Call: {info['call_vol']:,} • Put: {info['put_vol']:,}")# Aggregate
...
else: col2.info("**Neutral** options flow")with tab5:
    st.subheader(" Market Moving Events")# Dynamic Control for the timeframe
