...
# Build unified symbol → label mapping
symbol_to_label = {}
for ticker_dict in [GLOBAL_TICKERS, SECTOR_TICKERS, NEO_CLOUD_TICKERS, MAG7_TICKERS]:
    for label, sym in ticker_dict.items():
        if sym not in symbol_to_label:
            symbol_to_label[sym] = label

for theme_tickers in TRADING_THEMES.values():
    for sym in theme_tickers:
        if sym not in symbol_to_label:
            symbol_to_label[sym] = sym

ALL_SYMBOLS = list(symbol_to_label.keys())
ANALYST_SYMBOLS = sorted({sym for theme_tickers in TRADING_THEMES.values() for sym in theme_tickers})
...
def get_earnings_calendar_finnhub(date_str: str):
    ...
    for sym in ANALYST_SYMBOLS[:max_stocks]:
        try:
            ...
            for row in table.find_all("tr")[:8]:
                ...
                if len(title) < 25 or not is_high_impact(title):
                    continue
                link = a_tag.get("href")
...
def get_analyst_ratings():
    ...
    for sym in ANALYST_SYMBOLS:
        try:
            tk = yf.Ticker(sym)
            info = tk.info
            raw_key = info.get("recommendationKey")
            ...
            ratings.append({
                ...
                "Analyst Count": int(info.get("numberOfAnalystOpinions", 0))
            })
        except:
            continue
...
with tab_overview:
    ...
    st.dataframe(
        key_df.style.background_gradient(cmap='RdYlGn', subset=['Change %', 'Gap %', 'RVOL']),
        hide_index=True,
...
with tab_sectors:
    ...
    with col_b:
        ...
        st.dataframe(
            neo_data[['Asset', 'Price', 'Change %', 'RVOL']]
            .style.background_gradient(cmap='RdYlGn', subset=['Change %', 'RVOL']),
            hide_index=True,
            use_container_width=True
        )
...
with tab_rel_strength:
    ...
    try:
        ...
        melt_df = normalized_df.reset_index().melt(id_vars=normalized_df.index.name or 'Date', var_name='Ticker', value_name='Perf %')
        fig = px.line(melt_df, x=melt_df.columns[0], y='Perf %', color='Ticker', template="plotly_dark", height=500)
        fig.update_traces(line=dict(width=4, dash="dot"), selector=dict(name=benchmark))
    ...
    if user_ticker:
        try:
            ...
            if not options:
                ...
                with col1:
                    st.metric("🔄 **Gamma Flip Level**", f"${flip_level:,}", delta=f"Spot is {((spot - flip_level)/flip_level*100):+.1f}% above flip")
                with col2:
                    total_gex = round(df_agg.sum(), 1)
                    st.metric("Net GEX", f"{total_gex}M", delta="🟢 Long Gamma (pinning likely)" if total_gex > 0 else "🔴 Short Gamma (volatile)")
                with col3:
                ...
                fig.update_layout(
                    template="plotly_dark", title=f"{user_ticker} Net Gamma Exposure + Gamma Flip Level",
...
# ────────────────────────────────────────────────
#  REMAINING TABS (unchanged core logic, cleaner structure)
# ────────────────────────────────────────────────
with tab_options:
...
with tab_earnings:
    ...
    if all_events:
        ...
        def highlight_beats(val):
            ...
            return ''
        st.dataframe(df.style.map(highlight_beats, subset=['EPS Beat', 'Rev Beat']), hide_index=True, use_container_width=True)
...
with tab_analyst:
    ...
    if not analyst_df.empty:
        ...
        st.dataframe(
            ...
            .style.map(rating_color, subset=['Consensus'])
            .background_gradient(cmap='RdYlGn', subset=['Upside %', 'Bull Score'])
            .format({
                "Target Mean": "${:,.2f}", "Target High": "${:,.2f}", "Target Low": "${:,.2f}",
                "Current Price": "${:,.2f}", "Upside %": "{:+.1f}%"
            }),
            hide_index=True,
            use_container_width=True
        )
...
with tab_finnhub:
    ...
    st.caption("Live general market news + Today's Economic Calendar • Pre-market sentiment driver")
    ...
    with col2:
        ...
        if not econ_df.empty:
            ...
            st.caption("🔴 HIGH = major movers (CPI, FOMC, NFP, GDP, Unemployment, Rate Decision...)")
...
with tab_bias:
    ...
    st.dataframe(
        ...
        .style.map(style_bias, subset=['Bias'])
...
# Live auto-refresh
st_autorefresh(interval=45000, key="global_refresh")  # 45-second refresh
