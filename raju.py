...
def fetch_market_snapshot():
    ...
    # 1. Bulk Download ONLY (No Ticker loop)
    ...
    for sym in ALL_SYMBOLS:
        ...
        try:
            # Check if symbol exists in the downloaded data structure safely
            if 'Close' not in hist_data or sym not in hist_data['Close']:
                continue
                
            close_series = hist_data['Close'][sym].dropna()
            hist_vol_series = hist_data['Volume'][sym].dropna()
            ...
            # Extract today's intraday variables
            if 'Close' in intra_today and sym in intra_today['Close']:
                today_closes = intra_today['Close'][sym].dropna()
                today_vol_series = intra_today['Volume'][sym].dropna()
                today_opens = intra_today['Open'][sym].dropna()
            else:
                today_closes = pd.Series(dtype=float)
                today_vol_series = pd.Series(dtype=float)
                today_opens = pd.Series(dtype=float)

            if not today_closes.empty:
                price = float(today_closes.iloc[-1])
                today_open = float(today_opens.iloc[0])
                today_vol = float(today_vol_series.sum())
            else:
            ...
            prev_vol = float(hist_vol_series.iloc[-1]) if not hist_vol_series.empty else 1.0
...
def get_finnhub_econ_calendar():
    ...
    try:
        ...
        if df.empty: return pd.DataFrame(columns=["Time (ET)", "Event", "Actual", "Estimate", "Previous", "Country", "Impact"])
...
def get_finnhub_general_news():
    ...
    try:
        ...
        df = pd.DataFrame(items)
        if not df.empty: df = df.sort_values('Score', ascending=False)
        return df
    except:
        return pd.DataFrame()
...
with tab_rel_strength:
    ...
    try:
        ...
        fig = px.bar(
            df_plot, x='vs SPY (%)', y='Asset', orientation='h',
            color='vs SPY (%)', color_continuous_scale='RdYlGn',
    ...
    try:
        ...
        fig = px.bar(
            df_plot, x='vs QQQ (%)', y='Asset', orientation='h',
            color='vs QQQ (%)', color_continuous_scale='RdYlGn',
